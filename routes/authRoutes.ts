...
authRoutes.get("/signup/google/redirect", passport.authenticate("google"), (req, res) => {
  res.status(200).json({ "msg": "scuccess" });
})
...
authRoutes.post("/login", async (req: Request<{}, {}, { MobileNo: string,Email:string }, {}>, res: Response, next: NextFunction) => {
  if(req.body?.MobileNo){
    ...
    const otp = randomstring.generate({ length: 6, charset: 'numeric' });
    const expiresAt = Date.now() + 1 * 60 * 1000; // Set expiration time to 1 minutes
...
authRoutes.post("/login/verify", async (req: Request<{}, {}, { MobileNo: string, otp: string }, {}>, res: Response, next: NextFunction) => {
  if(req.body?.Mo)
  const MobileNo = req.body?.MobileNo;
  console.log("called")
